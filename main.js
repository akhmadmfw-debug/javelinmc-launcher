...
function freezeServerConfig(root, mc, loader, log) {
  try {
    ...
    if (fs.existsSync(настройкиИгрока)) {
      // Если своих модов у игрока ещё нет, то и своих настроек взяться неоткуда:
      // всё, что лежит в config, пришло от сервера. Убираем целиком.
      // Если моды есть — осторожничаем и трогаем только то, что в точности
      // повторяет серверную сборку.
      const своиМоды = listJars(path.join(папкаИгрока, "mods")).length;
      const убрать = своиМоды === 0
        ? fs.readdirSync(настройкиИгрока)
        : (fs.existsSync(серверная) ? fs.readdirSync(серверная) : []);
